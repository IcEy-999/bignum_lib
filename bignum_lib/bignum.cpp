...
bignum bignum_sub(bignum a, bignum b) {
	...
	for (ULONG32 i = 0; i < Number_of_cycles; i++) {
		sub = a.U64[i] - b.U64[i];
		if (sub >= a.U64[i] && i< Number_of_cycles-1) {//溢出
			ls.U64[i + 1] -= 1;
		}
		ls.U64[i] += sub;
		ls.set_len();
...
bignum bignum_div(bignum dividend, bignum divisor) {
	...
	int j = 0;
	if (bignum_cmp(divisor, ls) == 2 || bignum_cmp(dividend, ls) == 2)
		return ls;
...
bignum bignum_mod(bignum dividend, bignum divisor) {
	...
	bignum quotient_ls, quotient_num;
	ULONG32 j = 0;
	if (bignum_cmp(divisor, ls) == 2 || bignum_cmp(dividend, ls) == 2)//除数，被除数等于零，返回0
		return ls;
	if (bignum_cmp(dividend, divisor) == 2)
		return ls;
	while (bignum_cmp(remainder, divisor) == 1) {
...
bignum_div_r bignum_div_all(bignum dividend, bignum divisor) {
	bignum_div_r test;
	bignum ls, quotient, remainder = dividend;
	bignum quotient_ls, quotient_num;
	int j = 0;
	...
	test.div = quotient; test.mod = remainder;
	return test;
}
